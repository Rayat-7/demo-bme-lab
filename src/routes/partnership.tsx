  ...
  return (
    <main className="min-h-screen bg-brand-bg pb-32">
      ...
      <div className="max-w-[1400px] mx-auto px-6 mt-20">
        <p className="text-xl md:text-2xl font-medium text-brand-text/60 leading-relaxed max-w-4xl mb-16">
          {partnershipsIntro}
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {partnershipsData.map((partner, index) => {
            ...
            return (
              <div key={index} className={`group bg-brand-bg border border-brand-border p-10 md:p-14 rounded-[40px] transition-all duration-500 hover:shadow-2xl hover:border-brand-text/20 hover:-translate-y-2 flex flex-col h-full ${isFullWidth ? 'md:col-span-2 md:flex-row gap-12 items-center' : ''}`}>
                 
                 <div className={`shrink-0 flex items-start gap-6 ${isFullWidth ? 'w-full md:w-1/3 flex-col md:flex-row' : 'mb-8 flex-col'}`}>
                   {partner.logoUrl ? (
                     <div className="w-24 h-24 shrink-0 rounded-[32px] bg-brand-text flex items-center justify-center p-4 shadow-sm  group-hover:scale-110 transition-all duration-500">
                       <img src={partner.logoUrl} alt={partner.name} className="w-full h-full object-contain brightness-0 invert opacity-90" />
                     </div>
                   ) : (
                     <div className="w-20 h-20 shrink-0 rounded-[32px] bg-brand-text/5 flex items-center justify-center text-brand-text shadow-sm group-hover:bg-brand-text group-hover:text-brand-bg group-hover:scale-110 transition-all duration-500">
                       <Handshake className="w-10 h-10" />
                     </div>
                   )}
                   <h3 className={`font-medium tracking-tight text-brand-text leading-[1.2] ${isFullWidth ? 'text-3xl md:text-4xl mt-2 md:mt-0' : 'text-2xl mt-4'}`}>
                     {partner.name}
                   </h3>
                 </div>

                 <div className={`flex-1 ${isFullWidth ? 'border-t md:border-t-0 md:border-l border-brand-border/50 pt-8 md:pt-0 md:pl-12' : ''}`}>
                   <p className="text-lg font-medium text-brand-text/70 leading-relaxed">
                     {partner.description}
                   </p>
                 </div>
              </div>
