      ...
      <div className="max-w-[1400px] mx-auto px-6 mt-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {researchAreasData.map((area, index) => {
             ...
             return (
              <div key={index} className={`group border p-10 md:p-14 rounded-[40px] transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 flex flex-col h-full ${
                isSpecial ? 'bg-brand-text text-brand-bg border-transparent' : 'bg-brand-bg text-brand-text border-brand-border hover:border-brand-text/20'
                 ...
                 <div className="space-y-6 flex-grow">
                   <h3 className="text-3xl md:text-4xl font-medium tracking-tight leading-[1.2]">
                     {area.title}
                   </h3>
