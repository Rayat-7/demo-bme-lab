      ...
      <div className="max-w-[1400px] mx-auto px-6 mt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-6 md:gap-8">
          ...
          {[das, rakhi, sabiha].map((award, index) => award && (
            <div key={`small-${index}`} className="md:col-span-2 group bg-brand-bg border border-brand-border p-8 rounded-[32px] transition-all duration-500 hover:shadow-xl hover:border-brand-text/20 hover:-translate-y-1 flex flex-col h-full">
          ...
          {[joy, sathi].map((award, index) => award && (
            <div key={`large-${index}`} className="md:col-span-3 group bg-brand-bg border border-brand-border p-10 rounded-[32px] transition-all duration-500 hover:shadow-2xl hover:border-brand-text/20 hover:-translate-y-1 flex flex-col h-full">
               ...
               <div className="space-y-6 flex-grow flex flex-col justify-between">
                 <div>
                   <h3 className="text-2xl md:text-3xl font-medium tracking-tight text-brand-text leading-[1.3] mb-4">
                     {award.name}
                   </h3>
          ...
          {owsd && (
            <div className="md:col-span-6 group bg-brand-bg border border-brand-border p-10 md:p-16 rounded-[40px] transition-all duration-500 hover:shadow-2xl hover:border-brand-text/20 hover:-translate-y-2 flex flex-col md:flex-row gap-12 items-center">
               <div className="flex-1 space-y-8">
                 <div className="flex items-start gap-6 mb-2">
                   <div className="w-20 h-20 shrink-0 rounded-[32px] bg-brand-text flex items-center justify-center text-brand-bg shadow-xl group-hover:scale-110 transition-all duration-500">
                     <Trophy className="w-10 h-10" />
                   </div>
                   <div>
                     <h3 className="text-3xl md:text-5xl font-medium tracking-tight text-brand-text leading-[1.1] mb-4">
                       {owsd.name}
                     </h3>
                     <div className="flex items-center gap-4 text-brand-text/80">
                        <span className="w-12 h-[2px] bg-brand-text"></span>
                        <p className="text-xl font-bold tracking-widest uppercase">{owsd.recipient}</p>
                     </div>
                   </div>
                 </div>
                 
                 {owsd.description && (
                   <div className="text-brand-text/70 text-lg md:text-xl font-medium leading-relaxed space-y-4">
               ...
               <div className="w-full md:w-[400px] shrink-0 space-y-6">
                 {owsd.amount && (
                   <div className="p-8 rounded-3xl bg-brand-text/5 border border-brand-border/50 text-center group-hover:bg-brand-text/10 transition-colors duration-500">
                     ...
                     <p className="text-4xl font-medium text-brand-text">{owsd.amount}</p>
                   </div>
                 )}
                 {owsd.projectTitle && (
